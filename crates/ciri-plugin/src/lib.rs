//! Lua plugin engine for ciri.
//!
//! Provides a sandboxed Lua 5.4 VM with an event-driven API (`ciri.on`).
...
mod tests {
    ...
    fn format_tab_title_with_custom_handler() {
        ...
        engine
            ...
            .load(
                r#"
                ciri.on("format-tab-title", function(info)
    ...
    #[test]
    fn ciri_json_decode_is_available() {
        let engine = PluginEngine::new().unwrap();
        ...
        engine
            ...
            .load(
                r#"
                local parsed = ciri.json.decode('{"a":1,"b":[2,3]}')
                assert(parsed.a == 1)
                assert(parsed.b[1] == 2)
                assert(parsed.b[2] == 3)
                "#,
    ...
    #[test]
    fn ciri_json_encode_roundtrip() {
        let engine = PluginEngine::new().unwrap();
        let s: String = engine
            .lua
            .load(r#"return ciri.json.encode({a = 1, b = "two"})"#)
            .eval()
            .unwrap();
    ...
    fn user_format_usage_overrides_builtin() {
        ...
        engine
            ...
            .load(
                r#"
                ciri.on("format-usage", function(ctx)
