...
/// 2. User plugins from ~/.config/ciri/plugins/<name>/init.lua
/// 3. User init.lua from ~/.config/ciri/init.lua
...
fn user_plugins_dir() -> PathBuf {
    dirs::config_dir()
        .map(|d| d.join("ciri").join("plugins"))
        .unwrap_or_else(|| PathBuf::from("/tmp/.config/ciri/plugins"))
}

fn user_init_path() -> PathBuf {
    dirs::config_dir()
        .map(|d| d.join("ciri").join("init.lua"))
        .unwrap_or_else(|| PathBuf::from("/tmp/.config/ciri/init.lua"))
}
