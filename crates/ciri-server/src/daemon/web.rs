...
//! `/ws` to the binary ciri-protocol, all on the single `[web]` port.
//!
...
pub(crate) fn resolve_static_dir(configured: &str) -> Option<PathBuf> {
    ...
    let candidate = if trimmed.is_empty() {
        ...
        ciri_config::config::expand_config_path(trimmed)
    };
    match std::fs::canonicalize(&candidate) {
        Ok(c) if c.is_dir() => {
            log::info!("ciritty-server serving web UI from {}", c.display());
            Some(c)
        }
        _ => {
            log::warn!(
...
async fn spa_unavailable() -> Response {
    (
        ...
        "ciritty web UI is not installed on this server (set web.static_dir or place a \
...
mod router_tests {
    ...
    fn write_site(dir: &Path) {
        ...
        std::fs::write(dir.join("index.html"), "<!doctype html><title>ciritty</title>").unwrap();
    ...
    async fn serves_index_at_root_with_no_cache() {
        ...
        assert!(body.contains("ciritty"), "body was: {body}");
