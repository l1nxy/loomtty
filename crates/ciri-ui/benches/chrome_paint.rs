...
//!
//! Run all benches: `cargo bench -p ciri-ui --bench chrome_paint`.
//! Run one group:  `cargo bench -p ciri-ui --bench chrome_paint -- palette_paint`.
//! Run one bench:  `cargo bench -p ciri-ui --bench chrome_paint -- palette_paint/rows_30`.
...
fn bench_palette(c: &mut Criterion) {
    ...
    for &n in &[10usize, 30, 100] {
        group.bench_function(format!("rows_{}", n), |b| {
            ...
            let mut snapshot = ciri_ui::LayoutSnapshot::new();
...
fn bench_taffy_style_translation(c: &mut Criterion) {
    ...
    fn walk(el: &dyn ciri_ui::Element) {
