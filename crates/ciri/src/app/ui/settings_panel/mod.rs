...
use ciri_ui::{Banner, Div, Dropdown, NumberField, Severity, Styled, Switch, deferred, div, text};
...
pub(crate) struct SettingsPanelComponent {
    ...
    /// take `&CiriConfig`, and the panel needs a frozen view across
    /// the multiple build_tree() calls (capture / hit_test / paint).
    config_snapshot: CiriConfig,
}
    ...
    fn field_control(&self, cx: &UiContext<'_>, meta: &FieldMeta) -> Div {
        ...
        let inner = match meta.kind {
            ...
            FieldKind::Enum { .. } => {
                ...
                // long preset names ("ciri_dark" + checkmark prefix
