...
//! `CiriConfig`, how to **mutate** it (nudge / toggle / pick), how to
...
//! `ciri-app` layer can carry "which dropdown" without depending on
...
use ciri_config::config::{
    AnimationPreset, CenterStrategy, CiriConfig, InputMode, PredictionMode, StatusBarPosition,
    UiFontConfig,
};
...
pub static FIELDS: &[FieldMeta] = &[
    ...
    FieldMeta {
        ...
        kind: FieldKind::Enum {
            // Populated dynamically from `ciri_render::font_list::monospaced_families()` —
            // the empty slice here just signals "enum-typed" to the
            // panel; the actual variants come from `enum_variants()`.
            variants: &[],
        },
...
pub fn enum_variants(field: SettingsField) -> Vec<String> {
    if matches!(field, SettingsField::ThemePreset) {
        return ciri_config::theme::ThemeConfig::preset_names()
            .iter()
            .map(|s| (*s).to_string())
            .collect();
    }
    if matches!(field, SettingsField::FontFamily) {
        ...
        return ciri_render::font_list::all_families().to_vec();
    }
    if matches!(field, SettingsField::UiFontFamily) {
        ...
        out.extend(ciri_render::font_list::all_families().iter().cloned());
        return out;
    }
...
pub fn read_float(field: SettingsField, c: &CiriConfig) -> f32 {
    match field {
...
pub fn read_int(field: SettingsField, c: &CiriConfig) -> usize {
    match field {
...
pub fn read_bool(field: SettingsField, c: &CiriConfig) -> bool {
    match field {
...
pub fn read_enum(field: SettingsField, c: &CiriConfig) -> String {
    match field {
        SettingsField::ThemePreset => {
            ...
            // checkmark on the right row; "ciri_dark" is the documented
            ...
            ciri_config::theme::ThemeConfig::preset_names()
                .iter()
                .copied()
                .find(|n| *n == raw)
                .unwrap_or("ciri_dark")
...
pub fn display_value(field: SettingsField, c: &CiriConfig) -> String {
    match meta(field).kind {
...
/// gate redraws / writes).
pub fn nudge(field: SettingsField, c: &mut CiriConfig, delta_sign: i32) -> bool {
    match meta(field).kind {
...
pub fn toggle(field: SettingsField, c: &mut CiriConfig) -> Option<bool> {
...
/// actually changed.
pub fn set_enum(field: SettingsField, c: &mut CiriConfig, value: &str) -> bool {
...
fn write_float(field: SettingsField, c: &mut CiriConfig, v: f32) {
    match field {
...
fn write_int(field: SettingsField, c: &mut CiriConfig, v: usize) {
    match field {
...
fn write_bool(field: SettingsField, c: &mut CiriConfig, v: bool) {
    match field {
...
pub fn write_to_disk(field: SettingsField, c: &CiriConfig, w: &mut EditableConfig) {
    match field {
...
mod tests {
    ...
    fn write_to_disk_handles_every_field_without_panicking() {
        let cfg = CiriConfig::default();
        let path = std::env::temp_dir()
            .join("ciri-settings-schema-tests")
            .join(format!(
                "write-each-{}.toml",
        ...
        // validation test in ciri-config covers semantic correctness.
