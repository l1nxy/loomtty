...
/// matches `crates/ciri/src/connection.rs:387`, where the IO thread
/// builds its own runtime — keeps the usage probes independent of the
/// connection lifecycle so they keep working after disconnect.
pub fn spawn(config: PollerConfig, snapshot: SharedSnapshot) {
    let _ = std::thread::Builder::new()
        .name("ciri-usage-poller".into())
        .spawn(move || {
