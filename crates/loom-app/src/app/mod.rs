...
pub struct AppModel {
    ...
    /// Vertical scroll offset (in rows) for the settings-panel body.
    /// A category can list more fields than fit the panel height; the
    /// body windows its rows the same way the context menu does. Reset
    /// to 0 whenever the panel opens or the active category changes so
    ...
    pub cached_local_sessions: Vec<loom_protocol::message::SessionInfo>,
    /// Cached per-slot sessions from background slot queries.
    /// BTreeMap ensures deterministic iteration order by slot ID.
    pub cached_slot_sessions: BTreeMap<String, Vec<loom_protocol::message::SessionInfo>>,
...
impl AppModel {
    pub fn new(config: LoomConfig, session_name: impl Into<String>) -> Self {
        ...
        {
            use loom_input::keybind::{BindingSet, KeybindMap};
