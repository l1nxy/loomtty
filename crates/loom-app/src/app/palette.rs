...
impl AppModel {
            ...
            if let Some(probe) = self.cached_remote_probes.get(&rh.host) {
                match probe {
                    ...
                    RemoteProbeResult::NoServer => {
                        entries.push(PaletteEntry::new(
                            format!("SSH: {} (no loomtty-server)", rh.name),
    ...
    pub fn apply_slot_session_result(
        &mut self,
        slot_id: &str,
        sessions: Vec<loom_protocol::message::SessionInfo>,
    ) {
...
mod tests {
    ...
    use loom_config::config::{LoomConfig, RemoteHostConfig};
    use loom_layout::workspace_set::WorkspaceSet;
    use loom_protocol::message::{ClientMessage, SessionInfo};

    /// 创建一个最小的 AppModel 用于纯逻辑测试
    fn make_model() -> AppModel {
        AppModel::new(LoomConfig::default(), "test-session")
    }
