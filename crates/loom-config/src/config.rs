...
impl LoomConfig {
    ...
    /// preset resolution only. `loomtty web` uses this so it can still read
...
fn load_from_path(path: &PathBuf) -> Result<LoomConfig> {
    if path.exists() {
...
pub fn config_path() -> PathBuf {
    #[cfg(target_os = "macos")]
    {
        if let Some(home) = dirs::home_dir() {
            let legacy = home.join(".config").join("loom").join("config.toml");
            if legacy.exists() {
                return legacy;
            }
        }
    ...
    dirs::config_dir()
        ...
        .unwrap_or_else(|| {
            ...
            PathBuf::from(if cfg!(windows) {
                r"C:\nonexistent\.loom\config.toml"
            } else {
                "/nonexistent/.loom/config.toml"
            })
        })
}
...
mod tests {
    ...
    fn load_returns_defaults_when_config_is_missing() {
        ...
        assert_eq!(config.font.family, LoomConfig::default().font.family);
        assert_eq!(config.render.backend, LoomConfig::default().render.backend);
    }
    ...
    fn assert_validation_rejects(label: &str, mutate: fn(&mut LoomConfig)) {
    ...
    /// `load_lenient` (and thus `loomtty web`) must tolerate an `[web]`
    ...
    fn validate_rejects_out_of_range_values() {
        let cases: &[(&str, fn(&mut LoomConfig))] = &[
    ...
    fn load_ignores_unknown_fields() {
        ...
        assert_eq!(config.font.size, LoomConfig::default().font.size);
    }
    ...
    fn load_partial_sections_preserve_other_defaults() {
        ...
        let config = load_from_path(&path).unwrap();
        let defaults = LoomConfig::default();
    ...
    fn all_enum_variants_deserialize_from_toml() {
        ...
        for (section, key, value) in cases {
            ...
            let result: Result<LoomConfig, _> = toml::from_str(&toml_str);
    ...
    fn load_font_advanced_settings_round_trip() {
        ...
        "#;
        let cfg: LoomConfig = toml::from_str(toml_str).expect("parses");
