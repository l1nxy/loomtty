...
use loom_render::glyph_cache::PendingDwriteGlyph;
...
float4 ps_main(PSInput input) : SV_TARGET {
    ...
    return out_color * loom_corner_alpha(input.pane_local, pane_size, pane_radii);
}
"#;
...
// Matches loom_render::sdf_rect::SdfRect; offsets mirror the blade
...
// same Inigo Quilez SDF recipe, same shape as loom's existing
...
float loom_sdf_rounded_box(float2 p, float2 b, float4 r) {
...
float loom_corner_alpha(float2 px, float2 size, float4 radii) {
    if (radii.x <= 0.0 && radii.y <= 0.0 && radii.z <= 0.0 && radii.w <= 0.0) {
        return 1.0;
    }
    float d = loom_sdf_rounded_box(px - 0.5 * size, 0.5 * size, radii);
...
// Mirrors the blade SdfPipeline in crates/loom-gpu/src/blade.rs.
...
impl Renderer {
    pub fn new(window: Arc<Window>, render_config: &RenderConfig) -> Result<Self> {
        ...
        let sync_interval = match render_config.present_mode {
            loom_config::config::PresentMode::Immediate
            | loom_config::config::PresentMode::Mailbox => 0,
            _ => 1,
        };
    ...
    pub fn draw_frame(
        ...
        unsafe {
            ...
            if base_sdf_end < overlay_sdf_end {
                self.sdf
                    .draw_range(&self.ctx, base_sdf_end, overlay_sdf_end - base_sdf_end);
            }
            atlas_gpu.alpha.draw_batches(
                &self.ctx,
                alpha_count,
            ...
            if overlay_sdf_end < total_sdf {
                self.sdf
                    .draw_range(&self.ctx, overlay_sdf_end, total_sdf - overlay_sdf_end);
            }
            atlas_gpu.alpha.draw_batches(
                &self.ctx,
                alpha_count,
