...
impl Renderer {
    ...
    fn new_impl(window: Arc<Window>, render_config: &RenderConfig) -> crate::Result<Self> {
        ...
        let interval = match render_config.present_mode {
            ...
            | loom_config::config::PresentMode::Mailbox => SwapInterval::DontWait,
            loom_config::config::PresentMode::Fifo => {
    ...
    pub fn draw_frame(
        ...
        unsafe {
            ...
            let cc = if self.use_linear_blending {
                loom_config::theme::ThemeConfig::srgb_to_linear(scene.clear_color)
            } else {
                scene.clear_color
            };
...
void main() {
    ...
    frag_color *= loom_corner_alpha(v_pane_local, u_pane_size, u_pane_radii);
}
"#;
...
// distfunctions/). The same recipe powers loom's existing SDF chrome
...
float loom_sdf_rounded_box(vec2 p, vec2 b, vec4 r) {
...
float loom_corner_alpha(vec2 px, vec2 size, vec4 radii) {
...
void main() {
    ...
    color *= a;
    color *= loom_corner_alpha(v_pane_local, u_pane_size, u_pane_radii);
    frag_color = color;
}
"#;
