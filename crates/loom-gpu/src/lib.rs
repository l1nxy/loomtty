//! GPU backend crate for loom terminal renderer.
//!
...
///
/// Callers (e.g. loom-app) can match on variants to decide whether to
...
use loom_render::glyph_cache::GlyphCache;
use std::sync::Arc;
...
impl Renderer {
    ...
    pub fn new(window: Arc<Window>, render_config: &RenderConfig) -> Result<Self> {
        let backend_str = match render_config.backend {
            loom_config::config::RenderBackend::Auto => "auto",
            loom_config::config::RenderBackend::Blade => "blade",
            loom_config::config::RenderBackend::Gl => "gl",
        };
...
mod tests {
    ...
    fn draw_frame_backend_mismatch_is_safe() {
        ...
        let render_config = loom_config::config::RenderConfig::default();
        let mut cache =
        ...
        let scene = loom_render::FrameScene {
