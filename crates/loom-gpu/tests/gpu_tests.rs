//! GPU integration tests for loom-gpu (blade backend).
//!
...
//!
//! Set LOOM_SKIP_HEADLESS_GPU_TESTS to soft-skip these tests when headless GPU
...
use loom_render::glyph_cache::{GlyphInstance, PaneGlyphRange, PendingUpload};
use loom_render::rect::Rect;
use std::ptr;
...
fn create_headless_context() -> Option<gpu::Context> {
    match try_create_headless_context() {
        ...
        Err(e) if std::env::var("LOOM_SKIP_HEADLESS_GPU_TESTS").is_ok() => {
            eprintln!("Headless GPU init failed; skipping (LOOM_SKIP_HEADLESS_GPU_TESTS set): {e}");
            None
        }
        Err(e) => {
            panic!("Headless GPU init failed (set LOOM_SKIP_HEADLESS_GPU_TESTS to skip): {e}")
        }
    }
}
