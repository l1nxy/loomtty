...
use loom_render::shaper::{ShapingOptions, TextShaper};
use std::path::Path;
...
fn run_shape_substitutes_double_equals_glyphs() {
    ...
    // Drive the actual loom shaper over the family this file belongs to.
    ...
    let Some(font_id) = shaper.primary_font_id() else {
        eprintln!("loom shaper couldn't load Cascadia Code; skipping");
        return;
    };
...
fn assert_run_substitutes_input_glyph(family: &str, font_path: &str, text: &str, bare_ch: char) {
    ...
    let shaper = TextShaper::with_options(family, &ShapingOptions::default());
    let Some(font_id) = shaper.primary_font_id() else {
        eprintln!("loom shaper couldn't load {family}; skipping");
        return;
    };
...
fn render_double_plus_does_not_fall_back_to_bare_plus() {
    ...
    use loom_protocol::message::{CURSOR_HIDDEN, PackedCell};
