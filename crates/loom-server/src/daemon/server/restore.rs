...
impl Server {
    ...
    fn load_saved_session(session_name: &str) -> Option<loom_session::state::SessionState> {
        loom_session::restore::restore_session(session_name, &transport::state_dir())
            .ok()
            .flatten()
    }
    ...
    fn apply_restored_session(
        &mut self,
        session: &mut Session,
        saved: &loom_session::state::SessionState,
    ) {
    ...
    fn restore_workspace(
        &mut self,
        session: &mut Session,
        saved_ws: &loom_session::state::SavedWorkspace,
    ) -> loom_layout::workspace::Workspace {
        let mut ws = loom_layout::workspace::Workspace::new_with_gap(
    ...
    fn restore_column(
        &mut self,
        session: &mut Session,
        saved_col: &loom_session::state::SavedColumn,
    ) -> Option<loom_layout::column::Column> {
        ...
        let mut col_opt: Option<loom_layout::column::Column> = None;
        for saved_tile in &saved_col.tiles {
            ...
            let resume_cmd = saved_tile
                ...
                .and_then(|a| {
                    let cmd = loom_session::agent::resume_command(a.kind)?;
                    if cfg!(windows) {
    ...
    pub(super) fn push_tile_to_column(
        col_opt: &mut Option<loom_layout::column::Column>,
        pane_id: u64,
        width: ColumnWidth,
        weight: f64,
    ) {
        if let Some(col) = col_opt {
            let mut tile = loom_layout::tile::Tile::new(pane_id);
            tile.height = loom_layout::tile::TileHeight::Auto { weight };
            col.tiles.push(tile);
        } else {
            let mut col = loom_layout::column::Column::new(pane_id);
            col.width = width;
            if let Some(first_tile) = col.tiles.first_mut() {
                first_tile.height = loom_layout::tile::TileHeight::Auto { weight };
            }
            *col_opt = Some(col);
        }
    }
    ...
    fn init_default_workspace(
        ...
        session
            .workspaces
            .workspaces
            .push(loom_layout::workspace::Workspace::new_with_gap(vs, cg));
