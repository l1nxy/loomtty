...
impl Server {
    ...
    fn handle_list_sessions(&self, client_id: u64, all: bool, responses: &mut Vec<ServerResponse>) {
        ...
        if all {
            ...
            if let Ok(saved_names) = loom_session::restore::list_sessions(&transport::state_dir()) {
    ...
    /// gets the same bare-`loomtty` UX. The caller is responsible for
    ...
    fn handle_switch_session(
        ...
        if loom_session::names::validate_name(target).is_err() {
            responses.push(ServerResponse::SendToClient(
                client_id,
                ServerMessage::Error {
