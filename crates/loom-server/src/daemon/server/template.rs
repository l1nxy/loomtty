...
impl Server {
    ...
    fn handle_apply_template(
        ...
        use loom_session::template::{self, TemplateWidth};
                    ...
                    match Pane::new_with_notify(
                        ...
                        Ok(mut pane) => {
                            ...
                            if let Some(col) = &mut col_opt {
                                let mut tile = loom_layout::tile::Tile::new(id);
                                tile.height = loom_layout::tile::TileHeight::Auto {
                                    weight: tpl_tile.weight,
                                };
                                col.tiles.push(tile);
                            } else {
                                let mut col = loom_layout::column::Column::new(id);
                                col.width = ColumnWidth::Proportion(col_proportion);
                                if let Some(first_tile) = col.tiles.first_mut() {
                                    first_tile.height = loom_layout::tile::TileHeight::Auto {
                                        weight: tpl_tile.weight,
                                    };
                                }
                                col_opt = Some(col);
