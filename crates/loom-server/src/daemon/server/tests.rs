...
fn resolve_auto_session_creates_unique_name_when_no_sessions_exist() {
    ...
    assert!(
        loom_session::names::validate_name(&name).is_ok(),
...
fn control_attach_does_not_create_runtime_session() {
    ...
    rt.block_on(async {
        ...
        loom_protocol::codec::write_client_hello(&mut client_writer, &hello)
            .await
            .unwrap();
        client_writer.flush().await.unwrap();
        let compat = loom_protocol::codec::read_server_hello(&mut client_reader)
            .await
            .unwrap();
        assert!(matches!(
            compat,
            loom_protocol::codec::VersionCompat::Exact(_)
                | loom_protocol::codec::VersionCompat::PatchMismatch { .. }
                | loom_protocol::codec::VersionCompat::MinorMismatch { .. }
        ));
...
fn session_save_and_restore_roundtrip(mut server_with_session: (Server, String)) {
    ...
    {
        let session = server.sessions.get(session_name).unwrap();
        let state = loom_session::state::SessionState {
            name: session_name.clone(),
            workspaces: vec![loom_session::state::SavedWorkspace {
                columns: vec![loom_session::state::SavedColumn {
                    tiles: session
                        .panes
                        .keys()
                        .map(|&pid| loom_session::state::SavedTile {
                            pane_id: pid,
                            weight: 1.0,
        ...
        loom_session::save::save_session(&state, tmp.path()).unwrap();
    }

    // Verify file was written
    let saved = loom_session::restore::restore_session(session_name, tmp.path())
        .unwrap()
...
fn session_list_and_delete(mut server_with_session: (Server, String)) {
    ...
    // Save two sessions
    let state1 = loom_session::state::SessionState {
        name: session_name.clone(),
        workspaces: vec![loom_session::state::SavedWorkspace {
            columns: vec![loom_session::state::SavedColumn {
                tiles: vec![loom_session::state::SavedTile {
                    pane_id: 1,
                    weight: 1.0,
                    cwd: None,
                    title: None,
    ...
    loom_session::save::save_session(&state1, tmp.path()).unwrap();

    let state2 = loom_session::state::SessionState {
        name: "beta".to_string(),
        ..state1.clone()
    };
    loom_session::save::save_session(&state2, tmp.path()).unwrap();

    // List
    let names = loom_session::restore::list_sessions(tmp.path()).unwrap();
    assert!(names.contains(session_name));
    assert!(names.contains(&"beta".to_string()));

    // Delete one
    loom_session::restore::delete_session("beta", tmp.path()).unwrap();
    let names = loom_session::restore::list_sessions(tmp.path()).unwrap();
    assert!(!names.contains(&"beta".to_string()));
    assert!(names.contains(session_name));
}
...
async fn duplex_connect(
    ...
    loom_protocol::codec::write_client_hello(&mut cw, &hello)
        .await
        .unwrap();
    cw.flush().await.unwrap();
    let _ = loom_protocol::codec::read_server_hello(&mut cr)
        .await
        .unwrap();
    (cr, cw, state, handle)
}
...
async fn drain_initial_sync(cr: &mut tokio::io::BufReader<tokio::io::DuplexStream>) -> Vec<u64> {
    ...
    let pane_ids = match loom_protocol::codec::read_frame(cr).await.unwrap() {
        loom_protocol::codec::Frame::ServerMsg(ServerMessage::StateSync { pane_ids, layout }) => {
    ...
    for _ in &pane_ids {
        match loom_protocol::codec::read_frame(cr).await.unwrap() {
            loom_protocol::codec::Frame::FullPaneSync(sync) => {
    ...
    for _ in &pane_ids {
        match loom_protocol::codec::read_frame(cr).await.unwrap() {
            loom_protocol::codec::Frame::ServerMsg(ServerMessage::ImageDeleted { pane_id }) => {
                assert!(
...
async fn duplex_handshake_and_list_sessions() {
    ...
    loom_protocol::codec::encode_client_msg(&mut cw, &ClientMessage::ListSessions { all: false })
        .await
        .unwrap();
    cw.flush().await.unwrap();

    match loom_protocol::codec::read_frame(&mut cr).await.unwrap() {
        loom_protocol::codec::Frame::ServerMsg(ServerMessage::SessionList { sessions }) => {
...
async fn duplex_ping_pong() {
    ...
    loom_protocol::codec::encode_client_msg(
        &mut cw,
        &ClientMessage::Ping {
            seq: 99,
    ...
    match loom_protocol::codec::read_frame(&mut cr).await.unwrap() {
        loom_protocol::codec::Frame::ServerMsg(ServerMessage::Pong {
            seq,
            client_time_us,
        }) => {
            assert_eq!(seq, 99);
...
async fn duplex_reconnect_preserves_mutated_state() {
    ...
    loom_protocol::codec::encode_client_msg(&mut cw_a, &ClientMessage::CreatePane)
        .await
        .unwrap();
    cw_a.flush().await.unwrap();

    // Read PaneCreated to get the new pane ID
    let new_pane_id = match loom_protocol::codec::read_frame(&mut cr_a).await.unwrap() {
        loom_protocol::codec::Frame::ServerMsg(ServerMessage::PaneCreated { pane_id, .. }) => {
            pane_id
        }
        other => panic!("expected PaneCreated, got {other:?}"),
    };
    // Drain LayoutUpdate
    let _ = loom_protocol::codec::read_frame(&mut cr_a).await.unwrap();
...
async fn duplex_two_clients_same_session() {
    ...
    let frame_a = loom_protocol::codec::read_frame(&mut cr_a).await.unwrap();
    ...
    let frame_b = loom_protocol::codec::read_frame(&mut cr_b).await.unwrap();
    match frame_b {
        loom_protocol::codec::Frame::ServerMsg(ServerMessage::PaneCreated { pane_id, .. }) => {
    ...
    let frame_a2 = loom_protocol::codec::read_frame(&mut cr_a).await.unwrap();
    match frame_a2 {
        loom_protocol::codec::Frame::ServerMsg(ServerMessage::LayoutUpdate { layout }) => {
    ...
    let frame_b2 = loom_protocol::codec::read_frame(&mut cr_b).await.unwrap();
    match frame_b2 {
        loom_protocol::codec::Frame::ServerMsg(ServerMessage::LayoutUpdate { layout }) => {
            let total_panes: usize = layout
                .workspaces
