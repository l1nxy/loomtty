...
use loom_session::agent::SavedAgent;
use loom_session::save::save_session;
...
impl Session {
    ...
    pub(crate) fn pane_grid_size_with_cells(
        ...
        use loom_protocol::message::MAX_GRID_CELLS;
    ...
    fn detect_agent_for_pane(pane: &Pane) -> Option<SavedAgent> {
        ...
        let info =
            loom_procinfo::foreground_process(shell_pid, master_fd as loom_procinfo::RawHandle)?;
        loom_session::agent::detect_agent(&info.exe_name, &info.argv)
    }

    #[cfg(windows)]
    fn detect_agent_for_pane(pane: &Pane) -> Option<SavedAgent> {
        let shell_pid = pane.child_pid()?;
        let info = loom_procinfo::foreground_process(shell_pid, 0)?;
        loom_session::agent::detect_agent(&info.exe_name, &info.argv)
    }
...
mod tests {
    ...
    fn build_state_sync_emits_image_deleted_for_panes_without_active_images() {
        ...
        let image = loom_term::pane::ImagePlacement {
            id: 42,
            row: 1,
            col: 2,
            width_cells: 3,
