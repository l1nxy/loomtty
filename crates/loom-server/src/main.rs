...
fn main() -> Result<()> {
    ...
    if args.iter().any(|a| a == "--print-socket-path") {
        println!(
            "{}",
            loom_protocol::transport::server_socket_path().display()
        );
        return Ok(());
    }
...
fn parse_web_overrides(args: &[String]) -> Result<Option<loom_config::config::LoomConfig>> {
    ...
    let mut config = loom_config::config::LoomConfig::load()?;
    ...
    // Re-validate: schema checks already ran inside `LoomConfig::load`, but
...
fn daemonize() -> Result<()> {
    ...
    let log_dir = loom_protocol::transport::runtime_dir().join("loom");
