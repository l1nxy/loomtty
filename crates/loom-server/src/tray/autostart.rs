...
fn desktop_entry_path() -> Option<std::path::PathBuf> {
    dirs::config_dir().map(|d| d.join("autostart").join("loomtty-tray.desktop"))
}
    ...
    if enable {
        ...
        let content = format!(
            "[Desktop Entry]\n\
             Type=Application\n\
             Name=Loomtty Tray\n\
             Comment=System tray for loomtty terminal multiplexer\n\
             Exec={}\n\
             Hidden=false\n\
             NoDisplay=false\n\
...
fn launchd_plist_path() -> Option<std::path::PathBuf> {
    dirs::home_dir().map(|d| d.join("Library/LaunchAgents/dev.loomtty.tray.plist"))
}
...
<dict>
    <key>Label</key>
    <string>dev.loomtty.tray</string>
...
fn is_enabled_windows() -> bool {
    ...
    run.get_value::<String, _>("LoomttyTray").is_ok()
}
...
fn set_enabled_windows(enable: bool) -> Result<()> {
    ...
    if enable {
        let exe = std::env::current_exe()?;
        run.set_value("LoomttyTray", &exe.to_string_lossy().to_string())?;
        log::info!("wrote registry autostart entry");
    } else {
        let _ = run.delete_value("LoomttyTray");
