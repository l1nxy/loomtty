...
/// Stored as TOML in $XDG_CONFIG_HOME/loom/templates/<name>.toml
...
pub fn templates_dir() -> PathBuf {
    config_dir().join("loom").join("templates")
}
...
mod tests {
    ...
    fn unique_config_home(label: &str) -> PathBuf {
        std::env::temp_dir().join(format!("loom-session-{label}-{}", unique_suffix()))
    }
