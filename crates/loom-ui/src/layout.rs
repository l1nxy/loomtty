...
/// `hit_test` paths in loom's chrome widgets).
///
...
pub fn layout_tree_into_retained(
    ...
    if let Err(e) = layout_result {
        log::warn!("loom-ui: taffy compute_layout failed (hit-only): {e:?}");
        return;
    }
...
pub fn paint_tree_into_retained(
    ...
    if let Err(e) = layout_result {
        log::warn!("loom-ui: taffy compute_layout failed: {e:?}");
        return;
    }
...
fn paint_node<'a>(
    ...
    let layout = match tree.layout(node) {
        Ok(l) => l,
        Err(e) => {
            log::warn!("loom-ui: taffy layout query failed: {e:?}");
            return;
        }
    };
...
fn walk_for_layout_snapshot<'a>(
    ...
    let layout = match tree.layout(node) {
        Ok(l) => l,
        Err(e) => {
            log::warn!("loom-ui: taffy layout query failed (hit-only): {e:?}");
            return;
        }
    };
    let local_x = parent_local[0] + layout.location.x;
    ...
    fn paint_tree_with_layout_records_painted_bounds() {
        ...
        assert!(
            out.layout
                .nodes()
                .iter()
                .any(|n| n.type_id == "loom.div" && n.bounds == [10.0, 5.0, 50.0, 20.0]),
