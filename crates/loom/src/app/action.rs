...
impl App {
    ...
    pub(crate) fn rebuild_binding_set(input: &mut InputHandler, config: &LoomConfig) {
        ...
        match action {
            ...
            Action::CyclePresetWidth | Action::CyclePresetWidthReverse => {
                    ...
                    .cycle_preset_width(&presets, reverse)
                {
                    let (proportion, fixed_px) = match w {
                        loom_layout::column::ColumnWidth::Proportion(p) => (p, None),
                        loom_layout::column::ColumnWidth::Fixed(px) => {
                            let inner_vw =
            ...
            Action::NewSession => {
                let existing: Vec<String> = self
                    ...
                    .map(|s| s.name.clone())
                    .collect();
                let name = loom_session::names::unique_name(&existing);
