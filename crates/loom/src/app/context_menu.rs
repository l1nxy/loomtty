...
use loom_app::app::{ContextMenuParent, ModalKind, PendingPasteTarget};
                        ...
                        && let Ok(text) = cb.get_text()
                    {
                        ...
                        } else if let Some(pid) = self.core.workspaces.active_mut().active_pane_id()
                        {
                            let bracketed = self.core.pane_grids.get(&pid).is_some_and(|g| {
                                g.mode_flags & loom_protocol::message::MODE_BRACKETED_PASTE != 0
                            });
                            let mut data =
