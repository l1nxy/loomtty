use loom_app::app::{ModalKind, PendingPasteTarget};
use loom_input::action::Action;
use loom_input::keybind::BindingMode;
...
impl App {
                    ...
                    if self.core.broadcast_mode {
                        ...
                        for pid in visible_pids {
                            ...
                            let pane_wants = pane_kitty_flags
                                & loom_protocol::message::MODE_KITTY_REPORT_EVENTS
                                != 0;
                            if !pane_wants {
                                continue;
                            }
    ...
    fn encode_key_input(
        ...
        if kitty_flags & loom_protocol::message::MODE_KITTY_KEYBOARD != 0 {
            key_event_to_kitty_bytes(
                event,
                modifiers.ctrl,
                modifiers.shift,
            ...
            let app_cursor = mode_flags & loom_protocol::message::MODE_APP_CURSOR != 0;
            let app_keypad = mode_flags & loom_protocol::message::MODE_APP_KEYPAD != 0;
