...
use loom_render::glyph_cache::{GlyphCache, GlyphEntry, GlyphInstance, PaneGlyphRange};
...
pub(crate) struct RenderBuffers {
    ...
    /// cap.
    pub ui_sdf_rects: Vec<loom_render::sdf_rect::SdfRect>,
}
...
pub(crate) struct CachedUiScene {
    ...
    pub glyphs: Vec<GlyphInstance>,
    pub color_glyphs: Vec<GlyphInstance>,
    /// SDF-shader chrome rects emitted by loom-ui-painted widgets
    /// (rounded / bordered / shadowed).
    pub sdf_rects: Vec<loom_render::sdf_rect::SdfRect>,
...
pub(crate) struct UiFontInit {
    ...
    pub id: Option<loom_render::fontdb::ID>,
...
pub(crate) struct App {
    ...
    pub ui_shaper: Option<std::cell::RefCell<loom_render::ui_shaper::UiTextShaper>>,
    pub dpi_scale: f64,
    ...
    /// `Ctrl+Alt+P` (event.rs) or pre-enabled via `LOOM_DEBUG_PERF=1`.
    ...
    /// Pre-change `[web]` snapshot for the in-flight runtime web toggle. The
    /// panel persists + sends optimistically; if the daemon replies that the
    /// gateway failed to start (port taken, etc.) the `CommandResult` handler
    /// restores this so settings.toml and the panel don't claim a gateway is
    /// live when it isn't. Single slot — a rapid second web edit replaces it
    /// (last-pending-wins), which is fine for the click-then-wait common case.
    pub pending_web_rollback: Option<loom_config::schema::WebConfig>,
    ...
    pub drag: loom_app::app::ResizeDragState,
    ...
    pub gestures: loom_app::app::GestureState,
    ...
    /// Redraw-gating ticker for the new `loom-ui` animation layer.
    ///
    /// `loom-motion::AnimProp` instances call `wake()` / `sleep()` on
    ...
    pub motion_ticker: Arc<loom_motion::Ticker>,

    /// Persistent Taffy layout tree shared by every `loom-ui` paint /
    ...
    /// paint. See `loom_ui::paint_tree_into_with` for the mechanism.
    pub ui_taffy_tree: std::cell::RefCell<crate::app::ui::types::UiTaffyTree>,

    /// Bump arena for loom-ui Element trees. Cleared at the start of
    ...
    /// inside `Div::child` lands here. See `loom_ui::arena`.
    pub ui_arena: std::cell::RefCell<loom_ui::Arena>,
    ...
    pub ui_states: std::cell::RefCell<loom_ui::ElementStates>,
...
pub struct SettingsHoverCacheEntry {
    ...
    pub category: loom_app::app::SettingsCategory,
    pub visible: bool,
...
impl App {
    ...
    pub(crate) fn resolve_ui_font_init(config: &LoomConfig, dpi_scale: f64) -> UiFontInit {
        ...
        let (path, id) = match loom_render::ui_shaper::resolve_ui_font(family) {
            Some((p, idx, fid)) => {
                log::info!(
                    "UI font resolved: '{}' → {}",
                    if family.is_empty() {
    ...
    pub(crate) fn build_ui_shaper(
        ...
        cell_width: f32,
        cell_height: f32,
    ) -> loom_render::ui_shaper::UiTextShaper {
        use loom_render::ui_shaper::UiFontData;
    ...
    pub fn new(config: LoomConfig, session_name: impl Into<String>) -> Self {
        ...
        App {
            ...
            debug_metrics: {
                ...
                if std::env::var("LOOM_DEBUG_PERF")
                    .ok()
            ...
            motion_ticker: Arc::new(loom_motion::Ticker::new()),
            ...
            ui_arena: std::cell::RefCell::new(loom_ui::Arena::new(256 * 1024)),
            ui_states: std::cell::RefCell::new(loom_ui::ElementStates::new()),
    ...
    pub fn connect(
        &self,
        viewport: loom_protocol::codec::ClientHello,
    ) -> std::io::Result<(
        ...
        match self.connect(viewport) {
            ...
            Err(e) => {
                ...
                let reason = if e.kind() == std::io::ErrorKind::InvalidInput {
                    loom_app::app::DisconnectReason::InvalidTarget(e.to_string())
                } else if e.kind() == std::io::ErrorKind::NotFound {
                    loom_app::app::DisconnectReason::SshNotFound
                } else {
                    loom_app::app::DisconnectReason::SshSpawnFailed(e.to_string())
                };
    ...
    pub fn connect_remote_from_input(&mut self, input: &str) {
        ...
        let port = loom_protocol::transport::DEFAULT_REMOTE_PORT;
    ...
    pub fn start_remote_auto_connect(&mut self, host: String, port: u16, ssh_port: u16) {
        ...
        self.core.pending_auto_connect = Some(loom_app::app::PendingAutoConnect {
            host,
            port,
            ssh_port,
    ...
    pub fn preset_widths(&self) -> Vec<loom_layout::column::ColumnWidth> {
    ...
    pub(crate) fn current_paste_dialog_hover(&self) -> Option<loom_app::app::PasteButton> {
        ...
        if mx >= paste_left && mx < paste_right {
            Some(loom_app::app::PasteButton::Paste)
        } else if mx >= cancel_left && mx < cancel_right {
            Some(loom_app::app::PasteButton::Cancel)
        } else {
            None
        }
    }
    ...
    pub(crate) fn current_overview_action_hover(
        &self,
    ) -> Option<loom_app::app::OverviewActionHover> {
        if !self.core.overview.active {
            return None;
        }
        let bar =
        ...
        if mx >= close_left && mx < close_right {
            Some(loom_app::app::OverviewActionHover::Close)
        } else if mx >= separator_right && mx < focus_right {
            Some(loom_app::app::OverviewActionHover::Focus)
        } else {
            None
        }
    }
    ...
    pub(crate) fn current_top_bar_region_hover(&self) -> Option<loom_app::app::TopBarHoverRegion> {
        let (mx, my) = self.last_mouse_pos?;
        let (vw, vh) = self.command_palette_viewport_size();
        let (cw, ch) = self.cell_dimensions();
        let layout = self.top_bar_layout(vw, vh, cw, ch, self.ui_shaper.as_ref());
        ...
        if mx >= mode_left {
            return Some(loom_app::app::TopBarHoverRegion::Mode);
        }
        ...
        if !ws_label.is_empty() {
            ...
            if mx >= ws_left {
                return Some(loom_app::app::TopBarHoverRegion::Workspace);
            }
        }
        if mx < layout.session_w {
            return Some(loom_app::app::TopBarHoverRegion::Session);
        }
        None
    }
    ...
    pub fn total_chrome_width(&self) -> f32 {
        match self.core.config.tabbar.position {
            ...
            | loom_config::config::TabBarPosition::Right => self.core.config.tabbar.width,
        }
    }
    ...
    pub fn content_x_from_screen(&self, screen_x: f32, window_width: f32) -> Option<f32> {
        match self.core.config.tabbar.position {
            ...
            loom_config::config::TabBarPosition::Right => {
    ...
    pub fn side_tab_bar_rect(
        ...
        use loom_config::config::TabBarPosition;
    ...
    pub fn mark_disconnected_for_reconnect(&mut self, reason: loom_app::app::DisconnectReason) {
    ...
    pub fn prepare_reconnect(&mut self) -> Option<ReconnectPlan> {
        use loom_app::app::ReconnectPlanDecision;
    ...
    pub fn current_viewport(&self) -> loom_protocol::codec::ClientHello {
        let (cell_width, cell_height) = self.cell_dimensions();
        let view = &self.core.workspaces.view_size;
        loom_protocol::codec::ClientHello {
    ...
    pub fn preview_resize(&mut self, size: winit::dpi::PhysicalSize<u32>) {
        ...
        let center_strategy = match self.core.config.layout.center_focused_column {
            loom_config::config::CenterStrategy::Always => {
                loom_layout::workspace::CenterStrategy::Always
            }
            loom_config::config::CenterStrategy::OnOverflow => {
                loom_layout::workspace::CenterStrategy::OnOverflow
            }
            loom_config::config::CenterStrategy::Never => {
                loom_layout::workspace::CenterStrategy::Never
            }
        };
