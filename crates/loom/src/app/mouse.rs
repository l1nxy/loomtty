use loom_anim::spring::SpringParams;
use loom_protocol::message::ClientMessage;
use loom_protocol::message::{MODE_ALT_SCREEN, MODE_ALTERNATE_SCROLL, MODE_MOUSE_REPORT};
...
impl App {
        ...
        match hit {
            crate::app::ui::context_menu::ScrollbarHit::Thumb => {
                ...
                self.drag.context_menu_scrollbar_dragging =
                    Some(loom_app::app::ContextMenuScrollbarDrag {
                        start_my: my,
    ...
    fn forward_scroll_to_alt_screen(&mut self, dy: i32, pid: u64) {
        let app_cursor = self
            ...
            .pane_grids
            .get(&pid)
            .is_some_and(|g| g.mode_flags & loom_protocol::message::MODE_APP_CURSOR != 0);
...
mod tests {
    ...
    use loom_protocol::message::{MODE_ALT_SCREEN, MODE_MOUSE_REPORT};
