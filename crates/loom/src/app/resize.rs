...
pub(crate) struct ScrollbarHit {
    ...
    pub scrollbar_rect: Option<loom_render::rect::Rect>,
}
...
impl App {
    ...
    pub(crate) fn finish_resize_drag(&mut self) -> bool {
        ...
        if let Some((col_idx, top_tile_idx)) = self.drag.tile_dragging {
            ...
            if let Some(col) = ws.columns.get(col_idx) {
                ...
                if bot_idx < col.tiles.len() {
                    ...
                    self.send(loom_protocol::message::ClientMessage::SetTileWeights {
                        column_idx: col_idx,
                        top_tile_idx,
                        top_weight: top_w,
                        bottom_weight: bot_w,
        ...
        if let Some(drag_col) = self.drag.col_dragging {
            self.send(loom_protocol::message::ClientMessage::AdjustColumnSplitAt {
                column_idx: drag_col,
