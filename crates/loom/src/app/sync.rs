...
impl App {
    ...
    pub(crate) fn apply_font_config_change(&mut self) -> bool {
        ...
        if let Some(renderer) = &mut self.renderer {
            let shaper = loom_render::shaper::TextShaper::with_options(
                &self.core.config.font.family,
                &loom_render::shaper::ShapingOptions {
            ...
            let (cache, atlas_gpu) =
                match renderer.create_atlas(&loom_render::glyph_cache::FontInitParams {
            ...
            for event in events {
                match event {
                    ...
                    ServerEvent::Control(ServerMessage::PaneClosed { pane_id }) => {
                        ...
                        {
                            let geo = loom_anim::manager::GeoRect {
                                x: rect.x,
                                y: rect.y,
                                w: rect.w,
                                h: rect.h,
                    ...
                    | ServerEvent::Control(ServerMessage::PromptListReply { .. }) => {
                        ...
                        // clients in response to `loomtty msg …`). The GUI
...
mod tests {
    ...
    fn blank_full_sync(pane_id: u64, generation: u64, title: &str) -> FullPaneSyncBorrowed {
        ...
        loom_protocol::codec::full_pane_sync_to_borrowed(&sync).unwrap()
    }
    ...
    pub(crate) fn make_bg_slot(
        ...
        let view = loom_layout::geometry::ViewSize {
            width: 800.0,
            height: 600.0,
        };
    ...
    fn restore_from_slot_puts_pending_events_into_buffered() {
        ...
        let slot = super::super::ConnectionSlot {
            ...
            reconnect_state: None,
            pending_session_name: None,
            anim_mgr: loom_anim::manager::AnimationManager::new(),
            workspace_last_pane_ids: std::collections::HashMap::new(),
            selection: None,
            broadcast_mode: false,
            image_placements: std::collections::HashMap::new(),
