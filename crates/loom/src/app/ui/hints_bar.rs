...
use loom_ui::{Div, IntoElement, Render, RenderCtx, Styled, div, text};
...
impl HintsBarComponent {
    ...
    fn pick_hints(app: &App) -> Vec<(String, String)> {
        ...
        for (action, label) in [("toggle_lock", "lock"), ("toggle_help", "help")] {
...
impl HintsBarComponent {
    pub(crate) fn paint(&mut self, cx: &UiContext<'_>, scene: &mut UiScene<'_>) {
        ...
        // Fifth production usage of `loom_ui::Render`. The chrome
