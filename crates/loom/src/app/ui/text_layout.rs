...
mod tests {
    ...
    fn mk_cx(cw: f32) -> (UiContext<'static>, LoomConfig) {
        ...
        let cfg: &'static LoomConfig = Box::leak(Box::new(LoomConfig::default()));
        let theme: &'static loom_ui::ResolvedTheme =
            Box::leak(Box::new(loom_ui::ResolvedTheme::default()));
        let cx = UiContext {
            config: cfg,
            theme,
            viewport_w: 800.0,
