...
pub enum Command {
    ...
    Remote {
        ...
        /// Remote TCP port
        #[arg(long, default_value_t = loom_protocol::transport::DEFAULT_REMOTE_PORT)]
        port: u16,
    ...
    Web {
        ...
        /// listings — on a shared host prefer the LOOMTTY_WEB_TOKEN env var.
        #[arg(long)]
        token: Option<String>,
...
mod tests {
    ...
    fn attach_rejects_positional_session_name() {
        let err = Cli::try_parse_from(["loomtty", "attach"])
    ...
    fn no_args_help_flag_stays_on_safe_help_path() {
        let cli = Cli::try_parse_from(["loomtty", "--help"])
    ...
    fn positional_session_name_then_help_stays_on_safe_help_path() {
        let cli = Cli::try_parse_from(["loomtty", "work", "--help"])
            .expect_err("clap should exit after rendering help");
        assert_eq!(cli.kind(), clap::error::ErrorKind::DisplayHelp);
    }
