...
pub use loom_app::app::{DisconnectReason, RemoteProbeResult, RemoteQueryResult, ServerEvent};
...
pub fn connect_or_spawn(
    ...
    if let Err(reason) = loom_session::names::validate_name(session_name) {
...
/// Connect to a remote loom-server via SSH stdio proxy tunnel. `cancel` can
...
/// Fire-and-forget: probe a remote host for loom-server, query its sessions.
...
/// Probe whether a loomtty server is currently accepting connections.
///
...
/// `connect_or_spawn` and `loomtty web` so neither spawns a duplicate
