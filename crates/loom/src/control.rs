...
fn build_control_hello() -> Vec<u8> {
    ...
    hello.push(loom_protocol::codec::WIRE_PROTOCOL_VERSION);
    ...
    let mut stream = match stream_result {
        ...
        Err(_) => {
            ...
            if matches!(msg, ClientMessage::ListSessions { .. }) {
                ...
                let saved = loom_session::restore::list_sessions(&dir).unwrap_or_default();
                if saved.is_empty() {
                    println!("no sessions");
                } else {
            ...
            if let Ok(server_msg) = rmp_serde::from_slice::<ServerMessage>(&payload) {
                match server_msg {
                    ...
                    ServerMessage::PaneCapture {
                        ...
                        if truncated {
                            eprintln!(
                                "loomtty: capture-pane response was truncated to fit \
