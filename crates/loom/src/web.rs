//! `loomtty web` — turnkey browser access.
//!
...
pub fn run_web(
    ...
    // REPAIR it — `LoomConfig::load()` would reject it up front, leaving no
        ...
        .filter(|t| !t.is_empty())
    {
        log::warn!(
            "--token stays visible to other users in process listings (ps, \
             ...
             LOOMTTY_WEB_TOKEN environment variable, or omit --token to auto-generate one"
        );
        t
    } else if let Some(t) = std::env::var("LOOMTTY_WEB_TOKEN")
        .ok()
        .map(|t| t.trim().to_string())
        .filter(|t| !t.is_empty())
    {
    ...
    let static_dir = static_dir.map(|d| {
        let resolved = if d.starts_with("~/") || d.starts_with("~\\") {
            loom_config::config::expand_config_path(&d)
        } else {
    ...
    // actionable message, rather than a cryptic "loomtty-server exited"
    // after the fact.
    ...
    // `loomtty web` serves the SPA over http(s), so the browser always sends
    ...
    if config.remote.enabled && config.remote.port == port {
        bail!(
            "remote.port and web.port are both {port} — pick distinct ports \
             (defaults are 7890/7891), e.g. `loomtty web --port 7891`.",
        );
    }
    ...
    // Persist (comment-preserving) so the desktop app and later `loomtty
    ...
    if running {
        ...
        println!();
        println!("  A loomtty server is already running with its current settings:");
    ...
    // environment. Scrub LOOMTTY_WEB_TOKEN from the child so a token supplied
    // that way doesn't outlive this command in the long-running server's
    // /proc/<pid>/environ or a core dump.
    cmd.env_remove("LOOMTTY_WEB_TOKEN");
    let mut child = cmd
        .spawn()
    ...
    if !status.success() {
        bail!("loomtty-server exited with {status}");
    }
    Ok(())
}
...
mod tests {
    ...
    fn preflight_bind_rejects_an_occupied_port() {
        ...
        // unbindable — the check that stops `loomtty web` from persisting an
