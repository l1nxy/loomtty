...
| 之前说"没有" | 实际状态 | 证据 |
|---|---|---|
| 配置热重载 | **✅ 完整实现** | `crates/loom/src/app/event.rs:443-465` — `notify::recommended_watcher` + crossbeam channel + `reload_config()` |
| 桌面通知（OSC 9 / 99） | **✅ Unix 完整，Win 是 stub** | `crates/loom-term/src/pane/notify.rs:94,97` 解析 OSC 9/777；`crates/loom/src/app/notification.rs:25` 用 `notify_rust` 发送；`#[cfg(not(unix))]` 分支为空 |
| 标签页 | **✅ 横向 + 纵向两种** | `crates/loom/src/app/ui/tab_bar/mod.rs`（纵向左/右），`top_bar/pane_tabs.rs`（横向集成） |
| 命令完成时通知（含 exit code） | **✅ 已接通** | `crates/loom/src/app/sync.rs:224-253` — `ServerMessage::CommandCompleted { exit_code }` 触发通知 |
| Sticky 输入模式（zellij 风） | **✅ 已实现** | `crates/loom-input/src/leader/handler.rs:406` `InputMode::Sticky` |
...
| `loomtty new` | — | ✅ | 强制创建新 session |
| `loomtty init` | — | ✅ | 交互式配置向导 |
...
| `loomtty msg <sub>` | — | ✅ 8 个子命令 | 见下表 |
| `loomtty tpl <sub>` | `template` | ✅ 3 个子命令 | list/save/apply |
...
| 功能 | 状态 | 实现位置 |
|---|---|---|
| OSC 7 (cwd) | ✅ | `loom-term/src/osc7_parser.rs` |
| OSC 8 (hyperlinks) | ✅ 4096 link map | `loom-term/src/osc8_parser.rs` |
| OSC 9 (xterm notify) | ✅ | `loom-term/src/pane/notify.rs:94` |
| OSC 133 (prompt marks A/B/C/D) | ✅ 解析齐全 | `loom-term/src/shell_integration.rs` |
| OSC 777 (xterm notify ext) | ✅ | `loom-term/src/pane/notify.rs:97` |
| Kitty graphics protocol | ✅ 376 行 | `loom-term/src/kitty_graphics.rs` |
| Sixel | ✅ 613 行 | `loom-term/src/sixel.rs` |
| Inline image rendering | ✅ | `ServerMessage::ImagePlacement` + `render.rs:2112` |
| Kitty keyboard protocol | ✅ | `loom/src/app/key_encode/kitty.rs` |
| Legacy key encoding | ✅ | `loom/src/app/key_encode/legacy.rs` |
| Ligatures（含 fi / Arabic 合字） | ✅ 带缓存 | `loom-render/src/shaper.rs:76+` |
| 字体回退（cmap 解析） | ✅ | `loom-render/src/font_resolver/` |
| IME / CJK | ✅ | `loom/src/app/ime.rs` |
| 预测回显（mosh 风） | ✅ 三档：Never/Always/Adaptive | `loom-app/src/prediction/` |
| 滚动条 | ✅ | `loom-render/src/terminal/scrollbar.rs` |
| Bell（视觉+音频） | ✅ | `loom/src/app/ui/bell_flash.rs`, `notification.rs` 内 audio |
...
| 功能 | 状态 | 备注 |
|---|---|---|
| Blade 后端（Vulkan/Metal） | ✅ | `crates/loom-gpu/src/blade.rs` |
| OpenGL 后端 | ✅ | `gl.rs` |
| DX11 后端 | ✅ | `dx.rs` |
...
| 动画（bezier / spring / easing） | ✅ 4 个 preset | `crates/loom-anim/` |
...
| 列布局 + workspace | ✅ | 独家形态 |
| 模板（template）保存/加载 | ✅ | `loom-session/src/template.rs` |
...
| 功能 | 状态 | 备注 |
|---|---|---|
| bash 集成脚本 | ✅ | `shell-integration/loom.bash` |
| zsh 集成脚本 | ✅ | `loom.zsh` |
| fish 集成脚本 | ✅ | `loom.fish` |
...
| 功能 | 状态 |
|---|---|
| Claude OAuth 探测 | ✅ `crates/loom/src/app/usage/claude.rs` |
...
| 主题预设 | ✅ 8 个 | loom_dark / one_dark / catppuccin_mocha / tokyo_night / dracula / nord / gruvbox_dark / ghostty |
| 主题自定义（TOML 覆盖） | ✅ | |
| 导入 Alacritty 配色 | ❌ | |
...
| Windows procinfo 半成品 | `loom-procinfo/src/windows.rs:8,176,185` | Win 上 agent 自动恢复 / cwd 探测失效 |
...
loomtty **比我前几轮断言的要完整得多**。被我误判"缺失"的功能里：
