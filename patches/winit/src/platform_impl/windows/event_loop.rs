    ...
    let callback = || match msg {
        ...
        WM_DPICHANGED => {
            ...
            let new_outer_rect: RECT;
            {
                ...
                // Windows 11 (build >= 22000) provides a correct suggested_rect,
                // so the cursor-bias and nudge-loop workarounds are unnecessary
                // and actively cause snap-back issues when dragging between monitors.
                ...
                if !is_win11 && dragging_window {
                    // Windows 10: cursor-bias to preserve horizontal cursor position
                    let bias = {
                        let cursor_pos = {
                ...
                if is_win11 {
                    // Windows 11: use the OS-suggested rect position directly.
                    new_outer_rect = RECT {
                        left: suggested_rect.left,
                        top: suggested_rect.top,
                        right: suggested_rect.left + (conservative_rect.right - conservative_rect.left),
                        bottom: suggested_rect.top + (conservative_rect.bottom - conservative_rect.top),
                    };
                } else {
                    // Windows 10: nudge the rect onto the correct monitor if needed.
