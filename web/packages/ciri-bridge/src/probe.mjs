#!/usr/bin/env node
// ciri-bridge probe — sanity-checks the full wire path without a
// browser. Opens a WebSocket to the bridge, sends a real
// ClientHello via @ciri/client's encoder, waits for the first
// frame back from ciri-server.
//
// Use this to confirm the three-piece dev setup (ciri-server with
// `[remote] enabled = true` + bridge + reachable host) is correctly
// wired before opening the browser demo.
//
// Usage:
//   node packages/ciri-bridge/src/probe.mjs \
...
import { encodeClientHello } from "@ciri/client";
...
ws.on("message", (data) => {
  // Don't try to parse — the bridge transports raw ciri-protocol
