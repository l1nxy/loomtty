...
describe("decodeCellDelta", () => {
  ...
  test("rejects inverted region bounds (left > right)", () => {
    ...
    // Header is 43 bytes (pane_id 8, generation 8, cursor_line 2,
    // cursor_col 2, cursor_shape 1, mode_flags 2, received_ack 8,
    // echo_ack 8, cols 2, num_regions 2), then region header at offset
    // 43: line(2), left(2), right(2), sm_len(4).
    // left at offset 45, right at offset 47.
    bytes[45] = 5;
    bytes[46] = 0;
    bytes[47] = 4;
    bytes[48] = 0;
  ...
  test("rejects region extending past cols", () => {
    ...
    const bytes = hexToBytes(f.hex);
    // Same offsets as above — header 43 bytes, region 0 right at +4.
    bytes[47] = 80;
    bytes[48] = 0;
  ...
  test("rejects SM payload length past frame cap", () => {
    ...
    const f = CELL_DELTA_FIXTURES[0]!;
    const bytes = hexToBytes(f.hex);
    // sm_data_len lives at header(43) + line(2) + left(2) + right(2) = +49.
    bytes[49] = 0xff;
    bytes[50] = 0xff;
    bytes[51] = 0xff;
    bytes[52] = 0xff;
...
describe("decodeFullPaneSync", () => {
  ...
  test("rejects oversize grid", () => {
    ...
    // Header layout (the grid check runs after the whole header + title):
    //   pane_id(8), generation(8), cols(2), rows(2),
    //   cursor_line(2), cursor_col(2), cursor_shape(1), mode_flags(2),
    //   received_ack(8), echo_ack(8), title_len(2) = 45 bytes.
  ...
  test("rejects oversize scrollback", () => {
    ...
    //   header(45) + title_len(0) + scrollback_rows(4)
    //              + scrollback_replace(1) = 50 bytes minimum.
    const bytes = new Uint8Array(50);
    bytes[16] = 10; // cols low byte = 10
    bytes[18] = 1; // rows low byte = 1
    // scrollback_rows = u32::MAX → 4_294_967_295 * 10 ≫ 10M cap.
    // After the 45-byte header + empty title, scrollback_rows is at 45.
    bytes[45] = 0xff;
    bytes[46] = 0xff;
    bytes[47] = 0xff;
    bytes[48] = 0xff;
  ...
  test("rejects invalid UTF-8 in title", () => {
    ...
    const bytes = new Uint8Array(45 + 1 + 4 + 1 + 4 + 4 + 1 + 2 + 2 + 2 + 2);
    bytes[16] = 1; // cols low = 1
    bytes[18] = 1; // rows low = 1
    bytes[43] = 1; // title_len low = 1 (after the 45-byte header)
    bytes[45] = 0x80; // invalid UTF-8 start
