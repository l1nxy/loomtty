// LoomApp integration tests. We construct the app against a fake
...
function bootstrap(opts: { sessionName?: string } = {}) {
  ...
  let capturedEvent: ((e: LoomEvent) => void) | null = null;
  let started = false;
  let closed = false;
  const fakeClient: LoomAppClientLike = {
    start() {
      started = true;
    },
    close() {
...
afterEach(() => {
  ...
  // ".loom-app" / ".loom-orphan-panes" probes don't leak.
  document.body.replaceChildren();
});

describe("LoomApp lifecycle", () => {
  test("constructor mounts chrome + orphan root", () => {
    const { root } = bootstrap();
    expect(root.querySelector(".loom-app")).not.toBeNull();
    expect(document.body.querySelector(".loom-orphan-panes")).not.toBeNull();
  });
  ...
  test("destroy() closes client and clears DOM", () => {
    ...
    expect(document.body.querySelector(".loom-orphan-panes")).toBeNull();
  });
  ...
  test("destroy() unregisters root DOM listeners", () => {
    ...
    // removeEventListener'd. A SPA tearing LoomApp down and recreating
...
describe("LoomApp — layout + full-pane-sync", () => {
  test("LayoutUpdate then FullPaneSync mounts a renderer in the right slot", () => {
    ...
    const pane = tile!.querySelector(".loom-pane");
    expect(pane).not.toBeNull();
    expect(pane!.querySelectorAll(".loom-row").length).toBe(2); // rows=2
  ...
  test("FullPaneSync arriving before layout parks renderer in the orphan root", () => {
    ...
    expect(app.hasGrid(1n)).toBe(true);
    const orphans = document.querySelector(".loom-orphan-panes")!;
    expect(orphans.querySelector(".loom-pane")).not.toBeNull();
    // After a later LayoutUpdate, the renderer should move into its slot.
    fire({
      kind: "server-msg",
      msg: { tag: "LayoutUpdate", layout: mkLayoutSingle(1n) },
    });
    expect(orphans.querySelector(".loom-pane")).toBeNull();
    const tile = document.querySelector("[data-pane-id='1']")!;
    expect(tile.querySelector(".loom-pane")).not.toBeNull();
  });
...
describe("LoomApp — layout reshape moves renderers", () => {
  test("renderer container follows its pane to a new tile slot", () => {
    ...
    expect(tileA.querySelector(".loom-pane")).not.toBeNull();
    ...
    expect(tileA2.querySelector(".loom-pane")).not.toBeNull();
    expect(app.paneCount).toBe(1);
  });
  ...
  test("renderer of a now-inactive workspace is parked in orphan root", () => {
    ...
    const orphans = document.querySelector(".loom-orphan-panes")!;
    expect(orphans.querySelectorAll(".loom-pane").length).toBe(1);
    expect(document.querySelector("[data-pane-id='1']")).toBeNull();
    expect(app.paneCount).toBe(1);
  });
});

describe("LoomApp — keyboard routes to active pane", () => {
...
describe("LoomApp — mouse + wheel", () => {
  ...
  test("workspace tab click sends SwitchWorkspace", () => {
    ...
    const tabs = document.querySelectorAll<HTMLElement>(".loom-ws");
    tabs[1]!.click();
  ...
  test("wheel without Ctrl scrolls the targeted pane's scrollback", () => {
    ...
    expect(pane.querySelector(".loom-row")!.textContent).toBe("hi");
    ...
    expect(pane.querySelector(".loom-row")!.textContent).toBe("ok");
  ...
  test("scroll-to-bottom button shows when scrolled up and pins back to live", () => {
    ...
    const btn = root.querySelector<HTMLButtonElement>(".loom-scroll-bottom")!;
    const tile = document.querySelector<HTMLElement>("[data-pane-id='3']")!;
    const pane = tile.querySelector(".loom-pane")!;
...
describe("LoomApp — bell", () => {
  test("Bell adds .loom-tile-bell to the pane's tile and clears it after a timeout", async () => {
    vi.useFakeTimers();
    try {
      const { app, fire } = bootstrap();
      app.start();
      ...
      fire({ kind: "full-pane-sync", payload: hexToBytes(FULL_SYNC_3X2.hex) });
      const tile = document.querySelector<HTMLElement>("[data-pane-id='1']")!;
      expect(tile.classList.contains("loom-tile-bell")).toBe(false);
      fire({ kind: "server-msg", msg: { tag: "Bell", paneId: 1n } });
      expect(tile.classList.contains("loom-tile-bell")).toBe(true);
  ...
  test("Bell for a pane outside the active workspace is a no-op (no slot to flash)", () => {
    ...
    expect(document.querySelector(".loom-tile-bell")).toBeNull();
  });
...
describe("LoomApp — selection + clipboard", () => {
  ...
  function stubContainerRect(): void {
    vi.spyOn(Element.prototype, "getBoundingClientRect").mockImplementation(
      function (this: Element): DOMRect {
        if (this.classList?.contains("loom-pane")) {
          return {
  ...
  test("mousedown + drag inside one pane creates a selection range", () => {
    ...
    const rects = document.querySelectorAll(".loom-selection-row");
  ...
  test("mousedown + mouseup without movement clears any selection", () => {
    ...
    expect(document.querySelectorAll(".loom-selection-row").length).toBe(0);
  });
  ...
  test("Ctrl+Shift+C copies the selection's extracted text to the clipboard", async () => {
    ...
    const pane = slot.querySelector(".loom-pane") as HTMLElement | null;
    expect(pane).not.toBeNull();
    ...
    const root = document.querySelector(".loom-app")!.parentElement as HTMLElement;
    root.dispatchEvent(
  ...
  test("native paste event sends clipboardData text without the async API", () => {
    ...
    const sink = root.querySelector<HTMLTextAreaElement>(
      "textarea.loom-composition-sink",
    )!;
...
describe("LoomApp — error reporting", () => {
  ...
  test("close event surfaces via callback", () => {
    ...
    let captured: ((e: LoomEvent) => void) | null = null;
    const app = new LoomApp(root, {
      url: "ws://test",
      sessionName: "x",
      clientFactory: (_o, on) => {
...
describe("LoomApp — resize integration", () => {
  beforeEach(() => {
  ...
  test("Resize uses the workspace viewport rect, not the root (excludes tab strip)", () => {
    ...
    const root = app.layoutManager.viewportEl.closest(".loom-app")
  ...
  test("web-initiated CreatePane then ClosePane re-aims the lie back to single-pane real width", () => {
    ...
    const newBtn = document.querySelector<HTMLButtonElement>(
      '.loom-action[data-action-id="new-pane"]',
    )!;
    newBtn.click();
    fire({
      kind: "server-msg",
  ...
  test("subsequent LayoutUpdates do NOT spawn a fresh Resize even if proportions change (regression: CPU-burst feedback loop)", () => {
    ...
    // desktop loomtty on the same session) — bursting their render
  ...
  test("SwitchWorkspace flag re-aims the viewport lie at the new workspace's active pane", () => {
    ...
    const ws2Tab = document.querySelectorAll<HTMLButtonElement>(".loom-ws")[1]!;
    ws2Tab.click();
...
describe("LoomApp — IME composition", () => {
  ...
  test("compositionstart + update show the preedit on the active pane", () => {
    ...
    root.dispatchEvent(compositionEvent("compositionstart", ""));
    root.dispatchEvent(compositionEvent("compositionupdate", "ni"));
    const p = document.querySelector<HTMLElement>(".loom-preedit")!;
  ...
  test("compositionend resolves the commit target at commit time (Rust parity)", () => {
    // Rust's `Ime::Commit` path (crates/loom/src/app/ime.rs:36-41)
  ...
  test("mid-composition click + subsequent compositionupdate does NOT reroute (round-7 P1)", () => {
    ...
    const p1 = tile1After.querySelector<HTMLElement>(".loom-preedit")!;
  ...
  test("sink anchor honors scrollback offset (round-6 P3)", () => {
    ...
    fire({ kind: "full-pane-sync", payload: hexToBytes(FULL_SYNC_3X2.hex) });
    const tile = document.querySelector<HTMLElement>("[data-pane-id='1']")!;
    const pane = tile.querySelector<HTMLElement>(".loom-pane")!;
  ...
  test("LayoutUpdate with null active pane mid-composition clears IME state (round-9 P2)", () => {
    ...
    expect(document.querySelector(".loom-preedit")?.style.display).toBe("none");
  });
  ...
  test("LayoutUpdate during composition immediately transfers the preedit overlay (no compositionupdate needed)", () => {
    ...
    expect(
      tile1.querySelector<HTMLElement>(".loom-preedit")!.textContent,
    ).toBe("ni");
    ...
    const p2 = tile2After.querySelector<HTMLElement>(".loom-preedit")!;
    ...
    expect(
      tile1After.querySelector<HTMLElement>(".loom-preedit")!.style.display,
    ).toBe("none");
  });
  ...
  test("preedit overlay follows the active pane when LayoutUpdate changes focus mid-composition", () => {
    ...
    const p1Before = tile1.querySelector<HTMLElement>(".loom-preedit")!;
    ...
    const p1After = tile1After.querySelector<HTMLElement>(".loom-preedit")!;
  ...
  test("repositionCompositionSink prefers composingPaneId over pending click target (round-8 P1)", () => {
    ...
    const tile1 = document.querySelector<HTMLElement>("[data-pane-id='1']")!;
    const tile2 = document.querySelector<HTMLElement>("[data-pane-id='2']")!;
    const pane1 = tile1.querySelector<HTMLElement>(".loom-pane")!;
    const pane2 = tile2.querySelector<HTMLElement>(".loom-pane")!;
  ...
  test("destroyPane on the composing pane clears IME state", () => {
    ...
    root.dispatchEvent(compositionEvent("compositionend", "你"));
    expect(inputs.length).toBe(0);
    expect(document.querySelector(".loom-preedit")).toBeNull();
  });
  ...
  test("destroy() removes the composition sink from the root", () => {
    ...
    expect(
      root.querySelector("textarea.loom-composition-sink"),
    ).not.toBeNull();
    app.destroy();
    expect(root.querySelector("textarea.loom-composition-sink")).toBeNull();
  });
...
describe("LoomApp — chrome actions (workspace + session)", () => {
  test("workspace strip '+' sends SplitDown (new workspace)", () => {
    ...
    const btn = root.querySelector<HTMLButtonElement>(".loom-ws-new");
  ...
  test("session bar '+' creates and switches to a fresh session", () => {
    ...
    const btn = root.querySelector<HTMLButtonElement>(".loom-session-new");
    expect(btn).not.toBeNull();
    btn!.click();
  ...
  test("SessionList populates the dropdown (skipping __ sessions)", () => {
    ...
    const bar = root.querySelector<HTMLElement>(".loom-sessions")!;
    const select = root.querySelector<HTMLSelectElement>(".loom-session-select")!;
...
describe("LoomApp — mouse reporting (Phase 2.7-C)", () => {
  ...
  test("shift+mousedown on a mouse-mode pane falls back to text selection (bypass)", () => {
    ...
    expect(
      document.querySelectorAll(".loom-selection-row").length,
    ).toBeGreaterThan(0);
  });
  ...
  test("contextmenu: non-reporting pane shows our menu; mouse-reporting pane defers (Shift overrides)", () => {
    ...
    fire({ kind: "full-pane-sync", payload: hexToBytes(FULL_SYNC_3X2.hex) });
    const tile = document.querySelector<HTMLElement>("[data-pane-id='1']")!;
    const menu = root.querySelector<HTMLElement>(".loom-contextmenu")!;
...
describe("LoomApp — resize drag (Phase 2.7-A)", () => {
  ...
  function stubLayoutRects(opts: {
    ...
    vi.spyOn(Element.prototype, "getBoundingClientRect").mockImplementation(
      function (this: Element): DOMRect {
        if (this.classList?.contains("loom-workspace")) {
          return mkRect(0, 0, colWidths.reduce((a, b) => a + b, 0), 600);
        }
        if (this.classList?.contains("loom-column")) {
        ...
        if (this.classList?.contains("loom-tile")) {
  ...
  function setColumnFlex(idx: number, flex: number): void {
    const cols = document.querySelectorAll<HTMLElement>(".loom-column");
  ...
  test("hover near a column border sets col-resize cursor", () => {
    ...
    const ws = document.querySelector<HTMLElement>(".loom-workspace")!;
    ...
    const root = ws.closest<HTMLElement>(".loom-app")!.parentElement!;
    root.dispatchEvent(
      new MouseEvent("mousemove", { bubbles: true, clientX: 402, clientY: 100 }),
    );
  ...
  test("tile-border drag sends SetTileWeights with the new pair on mouseup", () => {
    ...
    const tiles = document.querySelectorAll<HTMLElement>(".loom-tile");
  ...
  test("column-resize clamp in a 3-column layout uses absolute 0.05 (not 5% of pair)", () => {
    ...
    cols.forEach((c) => { c.style.flex = String(1 / 3); });
    const root = document
      .querySelector<HTMLElement>(".loom-workspace")!
      .closest<HTMLElement>(".loom-app")!.parentElement!;
    root.dispatchEvent(
      new MouseEvent("mousedown", {
        bubbles: true,
        button: 0,
  ...
  test("server LayoutUpdate mid-drag cancels the resize cleanly (no stale node mutation on next move)", () => {
    ...
    const newCols = document.querySelectorAll<HTMLElement>(".loom-column");
  ...
  test("keydown that originates on a chrome button is NOT forwarded to the terminal", () => {
    ...
    // active. LoomApp's `onKeyDown` short-circuits when the event
    ...
    const chip = document.querySelector<HTMLButtonElement>(
      '.loom-pane-chip[data-chip-pane-id="1"]',
    )!;
    chip.dispatchEvent(
  ...
  test("resize started from outside-the-terminal focus moves focus back to the input sink", () => {
    ...
    // target. LoomApp re-queues the same focus call from the
    // capture handler.
    stubLayoutRects({});
    const { app, fire } = bootstrap();
    fire({
  ...
  test("hidden (display:none) columns do not contribute phantom resize borders mid-screen", () => {
    ...
    // `.loom-column { display: none }` hides inactive columns but
    ...
    vi.spyOn(Element.prototype, "getBoundingClientRect").mockImplementation(
      function (this: Element): DOMRect {
        ...
        if (this instanceof HTMLElement && this.classList?.contains("loom-column")) {
          if (this.classList.contains("loom-column-active")) {
            return mkRect2(0, 0, 800, 600);
          }
          return mkRect2(0, 0, 0, 0);
        }
        if (this instanceof HTMLElement && this.classList?.contains("loom-pane")) {
          return mkRect2(0, 0, 800, 600);
        }
        return mkRect2(0, 0, 0, 0);
      },
...
describe("LoomApp — clipboard receive / server errors / images / search", () => {
  ...
  test("Notification message shows an in-app toast", () => {
    ...
    const toasts = root.querySelectorAll(".loom-toast");
  ...
  test("SessionKilled shows a warning toast naming the session", () => {
    ...
    const toast = root.querySelector(".loom-toast");
    expect(toast).not.toBeNull();
    expect(toast!.classList.contains("loom-toast-warn")).toBe(true);
  ...
  test("CommandCompleted notifies only when the tab is hidden", () => {
    ...
    expect(root.querySelectorAll(".loom-toast").length).toBe(0);
    ...
    expect(toasts[0]!.classList.contains("loom-toast-warn")).toBe(true);
    Object.defineProperty(document, "hidden", { value: false, configurable: true });
  });
  ...
  test("tapping a toast dismisses it", () => {
    ...
    expect(root.querySelector(".loom-toast")).toBeNull();
  });
  ...
  test("ImagePlacement mounts an image canvas in the pane; ImageDeleted clears it", () => {
    ...
    expect(root.querySelector("canvas.loom-image")).not.toBeNull();
    fire({ kind: "server-msg", msg: { tag: "ImageDeleted", paneId: 1n } });
    expect(root.querySelector("canvas.loom-image")).toBeNull();
  });
  ...
  test("Ctrl+Shift+F opens the find bar and Escape closes it", () => {
    ...
    fire({ kind: "server-msg", msg: { tag: "LayoutUpdate", layout: mkLayoutSingle(1n) } });
    fire({ kind: "full-pane-sync", payload: hexToBytes(FULL_SYNC_3X2.hex) });
    const bar = root.querySelector<HTMLElement>(".loom-search")!;
    const input = root.querySelector<HTMLInputElement>(".loom-search-input")!;
  ...
  test("typing a query finds matches across viewport + scrollback", () => {
    ...
    ctrlShiftKey(root, "f");
    const input = root.querySelector<HTMLInputElement>(".loom-search-input")!;
    const status = root.querySelector<HTMLElement>(".loom-search-status")!;
    input.value = "hi";
  ...
  test("right-click over a pane opens the menu; Copy is disabled without a selection", () => {
    ...
    const menu = root.querySelector<HTMLElement>(".loom-contextmenu")!;
    expect(menu.hidden).toBe(false);
    const items = menu.querySelectorAll<HTMLButtonElement>(".loom-contextmenu-item");
  ...
  test("context menu Find item opens the search bar", () => {
    ...
    const findBtn = Array.from(
      root.querySelectorAll<HTMLButtonElement>(".loom-contextmenu-item"),
    ).find((b) => b.textContent === "Find…")!;
    findBtn.dispatchEvent(new MouseEvent("mousedown", { bubbles: true, cancelable: true }));
    expect(root.querySelector<HTMLElement>(".loom-search")!.hidden).toBe(false);
    // Menu closes after acting.
    expect(root.querySelector<HTMLElement>(".loom-contextmenu")!.hidden).toBe(true);
  });
});

describe("LoomApp — touch / mobile", () => {
  ...
  test("a tap focuses the composition sink (raises the soft keyboard)", () => {
    ...
    const sink = root.querySelector<HTMLTextAreaElement>("textarea.loom-composition-sink")!;
    sink.blur();
  ...
  test("long-press without drag opens the menu (Copy disabled)", () => {
    ...
    try {
      ...
      const menu = root.querySelector<HTMLElement>(".loom-contextmenu")!;
      expect(menu.hidden).toBe(false);
      const copy = menu.querySelector<HTMLButtonElement>(".loom-contextmenu-item")!;
