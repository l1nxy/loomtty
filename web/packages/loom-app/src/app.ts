...
//
// Lifecycle:
//   const app = new LoomApp(rootEl, { url, sessionName, ... });
...
/// Minimal client surface LoomApp consumes — everything the
/// orchestrator needs to drive a session. The default factory wraps
/// `LoomClient`; tests pass a custom factory that returns a fake.
export interface LoomAppClientLike {
  start(): void;
  close(): void;
...
export type LoomClientFactory = (
  opts: LoomClientOptions,
...
export interface LoomAppOptions {
  ...
  /// real `LoomClient`. Tests pass in a fake to capture outgoing
  /// messages and feed synthetic events back.
  clientFactory?: LoomClientFactory;
...
///
/// `id` strings are matched in `LoomApp.onAction` below — keep them
...
export class LoomApp {
  ...
  // would otherwise stick around forever and a remounted LoomApp on
  ...
  /// back as `LayoutUpdate`, and rebuilds every `.loom-column` /
  /// `.loom-tile` node mid-drag. Stopping propagation here keeps the
  ...
  constructor(
    ...
    this.orphanRoot.className = "loom-orphan-panes";
    ...
    this.compositionSinkEl.className = "loom-composition-sink";
    ...
    this.searchStatusEl.className = "loom-search-status";
    const mkSearchBtn = (label: string, aria: string, onClick: () => void) => {
      const b = doc.createElement("button");
      b.type = "button";
      b.className = "loom-search-btn";
      b.textContent = label;
    ...
    this.contextMenuEl.className = "loom-contextmenu";
    ...
    const mkMenuItem = (label: string, onClick: () => void): HTMLButtonElement => {
      const b = doc.createElement("button");
      b.type = "button";
      b.className = "loom-contextmenu-item";
    ...
    this.keyboardBtnEl.className = "loom-keyboard-toggle";
    ...
    this.scrollBottomEl.className = "loom-scroll-bottom";
    ...
    const clientOpts: LoomClientOptions = {
      url: opts.url,
      sessionName: opts.sessionName,
      viewport: initialViewport,
      ...(opts.token !== undefined ? { token: opts.token } : {}),
    };
    const factory: LoomClientFactory =
      opts.clientFactory ??
      ((o, h) => new LoomClient(o, h));
  ...
  start(): void {
    if (this.destroyed) throw new Error("LoomApp: start called after destroy");
    // Reset the one-shot initial-Resize gate so a `destroy()` →
    // `new LoomApp(...)` → `start()` cycle on the same root behaves
    // like a fresh attach.
  ...
  handleClientEvent(e: LoomEvent): void {
  ...
  private onTouchPointerDown(e: PointerEvent): void {
    ...
    const tileEl = e.target.closest(".loom-tile");
  ...
  /// (`crates/loom/src/app/resize.rs`) so feel is consistent across
  /// renderers.
  ...
  private hitTestResizeBorder(
    ...
    for (let i = 0; i < visibleColumns.length - 1; i += 1) {
      ...
      if (
        inVerticalSpan &&
        Math.abs(e.clientX - borderX) < LoomApp.RESIZE_HIT_PX
      ) {
        return {
          kind: "column",
          leftEl: left.el,
    ...
    for (const col of visibleColumns) {
      ...
      for (let ti = 0; ti < visibleTiles.length - 1; ti += 1) {
        ...
        if (Math.abs(e.clientY - borderY) < LoomApp.RESIZE_HIT_PX) {
          return {
            kind: "tile",
            topEl: top.el,
            bottomEl: bot.el,
  ...
  private applyResizeDrag(e: MouseEvent): boolean {
    ...
    if (drag.kind === "column") {
      ...
      // (`crates/loom-layout/src/workspace.rs:749`):
    ...
    // (`crates/loom-layout/src/workspace.rs:792-794`): a hard 30px
  ...
  private handleContextMenu(e: MouseEvent): void {
    ...
    const tileEl = target.closest(".loom-tile");
    if (!(tileEl instanceof HTMLElement)) return;
    const paneIdStr = tileEl.dataset["paneId"];
    if (paneIdStr === undefined) return;
    const paneId = BigInt(paneIdStr);
  ...
  /// Server-side dim cap (`crates/loom-server/.../session_mgmt.rs`
  ...
  private computeLiedViewportFor(targetPaneId: bigint | null): {
    ...
    //     (`crates/loom-layout/src/column.rs:tile_rects`). Mirror
    //     that for `p_tile`.
    let pCol = 1;
    let pTile = 1;
    let found = false;
  ...
  /// `crates/loom/src/app/render.rs` — search for `ime_input_anchor`).
  ///
  ...
  /// client's in-app switcher (`loom-app` palette, `app/mod.rs`,
  ...
  private handleServerMsg(msg: ServerMessage): void {
    switch (msg.tag) {
      ...
      case "ClipboardStore":
        ...
        // server's loom-term layer base64-decodes the OSC sequence.
  ...
  private handleBell(paneId: bigint): void {
    ...
    if (slot === null) return;
    slot.classList.add("loom-tile-bell");
    ...
    const t = win.setTimeout(() => {
      slot.classList.remove("loom-tile-bell");
  ...
  private showToast(text: string, kind: "info" | "warn" = "info"): void {
    ...
    el.className = `loom-toast loom-toast-${kind}`;
  ...
  private applyLayout(layout: LayoutState): void {
    ...
    // `setLayout` rebuilds every `.loom-column` / `.loom-tile` node —
    ...
    if (this.composing && this.composingPaneId !== null) {
      ...
      // `crates/loom/src/app/render.rs`. Round-12 codex P3.
      if (this.composing) this.repositionCompositionSink();
    }
