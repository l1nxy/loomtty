...
//
//   <div class="loom-app">
//     <div class="loom-workspaces">                  ← workspace tab strip
//       <button class="loom-ws[-active]" data-idx>...</button>
//     </div>
//     <nav class="loom-panes" role="toolbar" aria-label="Switch active pane">
//       <button class="loom-pane-chip"             ← pane switcher chips
//               type="button"
//               data-chip-pane-id
//               aria-current="true|false">title</button>
//       ...
//     </nav>
//     <div class="loom-workspace">                   ← active workspace content
//       <div class="loom-column[-active]" data-idx style="flex: <prop>">
//         <div class="loom-tile[-active]" data-pane-id style="flex: <weight>">
...
//   </div>
//
// The pane switcher (`.loom-panes`) is a primary mobile / narrow-
...
// or input. The caller (LoomApp) reads `getSlot(paneId)` to discover
...
export class LayoutManager {
  ...
  constructor(
    ...
    this.chrome.className = "loom-app";
    ...
    this.sessionBar.className = "loom-sessions";
    this.sessionBar.hidden = true;
    this.sessionSelect = doc.createElement("select");
    this.sessionSelect.className = "loom-session-select";
    ...
    newSessionBtn.className = "loom-session-new";
    ...
    this.wsTabs.className = "loom-workspaces";
    this.paneBar = doc.createElement("nav");
    this.paneBar.className = "loom-panes";
    ...
    this.wsContent.className = "loom-workspace";
  ...
  setLayout(layout: LayoutState): void {
    ...
    for (let i = 0; i < layout.workspaces.length; i += 1) {
      ...
      btn.className =
        i === activeWsIdx ? "loom-ws loom-ws-active" : "loom-ws";
    ...
    newWsBtn.className = "loom-ws-new";
    ...
    for (let ci = 0; ci < ws.columns.length; ci += 1) {
      ...
      colEl.className =
        ci === activeColIdx ? "loom-column loom-column-active" : "loom-column";
      ...
      // a "one-pane-per-screen" theme that wants `.loom-column { display: none }`
      ...
      for (let ti = 0; ti < col.tiles.length; ti += 1) {
        ...
        tileEl.className = isActive ? "loom-tile loom-tile-active" : "loom-tile";
        ...
        // wrapping `<span class="loom-pane-chip-group">` keeps the
        ...
        closeBtn.className = "loom-pane-close";
    ...
    if (chips.length > 0 && actions.length > 0) {
      ...
      divider.className = "loom-actions-divider";
  ...
  setTileTitle(paneId: bigint, title: string): void {
    ...
    const chip = this.paneBar.querySelector<HTMLElement>(
      `.loom-pane-chip[data-chip-pane-id="${paneIdStr}"]`,
    );
    if (chip !== null) {
    ...
    const closeBtn = this.paneBar.querySelector<HTMLElement>(
      `.loom-pane-close[data-close-pane-id="${paneIdStr}"]`,
    );
    if (closeBtn !== null) {
