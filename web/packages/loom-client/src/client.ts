// High-level loomtty WebSocket client.
//
...
export class LoomClient {
  private _state: LoomClientState = "idle";
  ...
  constructor(
    private readonly opts: LoomClientOptions,
    private readonly onEvent: (e: LoomEvent) => void,
  ) {
    this.hello = {
  ...
  start(): void {
    if (this._state !== "idle" && this._state !== "closed") {
      throw new Error(`LoomClient.start: bad state ${this._state}`);
    }
