// Low-level WebSocket adapter for the loomtty web gateway.
//
...
// routing — live in `LoomClient`. This adapter doesn't speak the
// loomtty wire protocol; it just shuttles bytes.
...
export interface WebSocketTransportOptions {
   ...
   *  `?token=...` query parameter (mirrors what `loom-server`'s WS
   *  handshake accepts). */
  url: string;
...
export class WebSocketTransport {
  ...
  private handleMessage(data: ArrayBuffer | string): void {
    if (typeof data === "string") {
      // The loom gateway never speaks text frames — protocol violation.
  ...
  private composeUrl(): string {
    ...
    // (`extract_token_query` in `crates/loom-server/src/daemon/ws.rs`)
