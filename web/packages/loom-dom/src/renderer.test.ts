...
describe("PaneRenderer", () => {
  test("mounts a wrapper under the root", () => {
    ...
    expect(root.querySelector(".loom-pane")).not.toBeNull();
    root.remove();
  });
  ...
  test("renders one row per grid row after a FullPaneSync", () => {
    ...
    r.render(grid);
    const rows = root.querySelectorAll(".loom-row");
  ...
  test("renders adjacent same-style cells into a single span", () => {
    ...
    r.render(grid);
    const row = root.querySelector(".loom-row");
  ...
  test("dirty-row diffing only touches the changed row", () => {
    ...
    r.render(grid);
    const beforeRows = Array.from(root.querySelectorAll(".loom-row"));
    ...
    const afterRows = Array.from(root.querySelectorAll(".loom-row"));
  ...
  test("bare URLs in plain text are autolinked as <a target=_blank>", () => {
    ...
    expect(root.querySelector(".loom-row")!.textContent).toBe(text);
    root.remove();
  });
  ...
  test("resize shrinks row containers", () => {
    ...
    r.render(grid);
    expect(root.querySelectorAll(".loom-row").length).toBe(3);
    grid.applyFullPaneSync(blankSync(2, 1));
    r.render(grid);
    expect(root.querySelectorAll(".loom-row").length).toBe(1);
    root.remove();
  });
  ...
  test("destroy detaches the wrapper and is idempotent", () => {
    ...
    expect(root.querySelector(".loom-pane")).not.toBeNull();
    r.destroy();
    expect(root.querySelector(".loom-pane")).toBeNull();
    r.destroy(); // no throw
    root.remove();
  });
  ...
  test("scroll offset 0 (default) shows the live viewport, not scrollback", () => {
    ...
    r.render(grid);
    const rows = Array.from(root.querySelectorAll(".loom-row"));
    expect(rows.length).toBe(2);
  ...
  test("setScrollOffset shifts display into scrollback", () => {
    ...
    r.setScrollOffset(2);
    r.render(grid);
    const rows2 = Array.from(root.querySelectorAll(".loom-row"));
  ...
  test("setScrollOffset is a no-op when value matches current offset", () => {
    ...
    const child0Before = root.querySelector(".loom-row")!.children[0]!;
    r.setScrollOffset(1);
    r.render(grid);
    const child0After = root.querySelector(".loom-row")!.children[0]!;
  ...
  test("appending scrollback under a scrolled-up user keeps the same content visible", () => {
    ...
    r.render(grid);
    expect(root.querySelector(".loom-row")!.textContent).toBe("aa");
  ...
  test("appending scrollback while pinned at bottom keeps following live", () => {
    ...
    r.render(grid);
    expect(root.querySelector(".loom-row")!.textContent).toBe("CC");
    expect(r.scrollOffsetRows).toBe(0);
    root.remove();
  });
  ...
  test("scrollback shrink (eviction) pulls scrollOffset back to keep view", () => {
    ...
    expect(root.querySelector(".loom-row")!.textContent).toBe("bb");
  ...
  test("cursor element exists and is initially hidden until first render", () => {
    ...
    document.body.appendChild(root);
    new PaneRenderer(root);
    const c = root.querySelector(".loom-cursor") as HTMLElement;
  ...
  test("render positions cursor at (cursorCol, cursorLine) of the live viewport", () => {
    ...
    expect(c.className).toContain("loom-cursor-block");
    root.remove();
  });
  ...
  test("cursor shape switches CSS class and styling", () => {
    ...
    for (const [shape, name] of [
      ...
      r.render(g);
      const c = root.querySelector(".loom-cursor") as HTMLElement;
      expect(c.className).toBe(`loom-cursor loom-cursor-${name}`);
    }
    root.remove();
  });
  ...
  test("CellDelta cursor move repositions the overlay without a full repaint", () => {
    ...
    grid.applyFullPaneSync(sync);
    r.render(grid);
    const cursor = root.querySelector(".loom-cursor") as HTMLElement;
  ...
  test("setSelection on a single row paints one .loom-selection-row rect", () => {
    const root = document.createElement("div");
    document.body.appendChild(root);
    const r = new PaneRenderer(root, { cursorBlink: false });
    ...
    r.render(grid);
    const rows = root.querySelectorAll(".loom-selection-row");
  ...
  test("multi-row selection paints three rects with correct slices", () => {
    ...
    r.render(grid);
    const rows = root.querySelectorAll<HTMLElement>(".loom-selection-row");
  ...
  test("setSelection(null) clears all rects on next render", () => {
    ...
    r.render(grid);
    expect(root.querySelectorAll(".loom-selection-row").length).toBe(1);
    r.setSelection(null);
    r.render(grid);
    expect(root.querySelectorAll(".loom-selection-row").length).toBe(0);
    root.remove();
  });
  ...
  test("selection rows outside the visible viewport are skipped", () => {
    ...
    expect(root.querySelectorAll(".loom-selection-row").length).toBe(2);
    root.remove();
  });
  ...
  test("theme.selectionBackground is honored", () => {
    ...
    r.render(grid);
    const rect = root.querySelector<HTMLElement>(".loom-selection-row")!;
  ...
  test("destroy clears selection state and removes overlay rects", () => {
    ...
    r.render(grid);
    expect(root.querySelector(".loom-selection-row")).not.toBeNull();
    r.destroy();
    expect(document.querySelector(".loom-selection-row")).toBeNull();
  ...
  test("preedit element exists and is initially hidden", () => {
    ...
    document.body.appendChild(root);
    const r = new PaneRenderer(root);
    const p = root.querySelector(".loom-preedit") as HTMLElement;
  ...
  test("setPreedit width counts CJK as double-width", () => {
    ...
    r.setPreedit("你好");
    r.render(grid);
    const p = root.querySelector<HTMLElement>(".loom-preedit")!;
  ...
  test("setPreedit(null) hides the overlay on next render", () => {
    ...
    r.render(grid);
    expect(
      (root.querySelector(".loom-preedit") as HTMLElement).style.display,
    ).toBe("block");
    r.setPreedit(null);
    r.render(grid);
    const p = root.querySelector(".loom-preedit") as HTMLElement;
  ...
  test("destroy clears preedit state", () => {
    ...
    r.setPreedit("zh");
    r.render(grid);
    expect(root.querySelector(".loom-preedit")).not.toBeNull();
    r.destroy();
  ...
  test("inline image is dropped when scrollback is replaced (epoch bump)", () => {
    ...
    r.render(grid);
    placeImage(r, grid, 0);
    expect(root.querySelectorAll("canvas.loom-image").length).toBe(1);
    ...
    r.render(grid);
    expect(root.querySelectorAll("canvas.loom-image").length).toBe(0);
    root.remove();
  });
