...
///
/// Owns:
///   - one wrapper `<div class="loom-pane">` mounted under `root`
///   - per-row `<div class="loom-row">` children, indexed by grid row
///
/// Does NOT own:
...
export class PaneRenderer {
  ...
  /// `crates/loom/src/app/ime.rs:45-48`). The browser DOM
  ...
  /// click) we add `loom-scrollbar-active`, reset this timer, and
  ...
  constructor(
    ...
    this.wrapper.className = "loom-pane";
    ...
    this.cursorEl.className = "loom-cursor";
    ...
    this.preeditEl.className = "loom-preedit";
    ...
    // restyle via `.loom-scrollbar` / `.loom-scrollbar-thumb`.
    this.scrollbarEl = this.doc.createElement("div");
    this.scrollbarEl.className = "loom-scrollbar";
    ...
    this.scrollbarThumbEl.className = "loom-scrollbar-thumb";
    ...
    // because `.loom-pane` doesn't have `overflow: scroll`; what
  ...
  /// shape as `clampNumber` in `@loom/app`.
  ...
  private onScrollbarThumbPointerDown(e: PointerEvent): void {
    ...
    this.scrollbarEl.classList.add("loom-scrollbar-dragging");
  ...
  private onScrollbarThumbPointerUp(e: PointerEvent): void {
    ...
    this.scrollbarEl.classList.remove("loom-scrollbar-dragging");
  }
  ...
  private revealScrollbar(): void {
    ...
    this.scrollbarEl.classList.add("loom-scrollbar-active");
    ...
    this.scrollbarHideTimer = setTimeout(() => {
      ...
      this.scrollbarEl.classList.remove("loom-scrollbar-active");
  ...
  private onPanePointerDown(e: PointerEvent): void {
    ...
    if (
      ...
      e.target.closest(".loom-scrollbar") !== null
    ) {
      return;
    }
  ...
  private updateCursor(grid: PaneGrid): void {
    ...
    this.cursorEl.className = `loom-cursor loom-cursor-${cursorShapeName(cursorShape)}`;
  ...
  private updateSelection(grid: PaneGrid): void {
    ...
    for (let srcRow = first.srcRow; srcRow <= last.srcRow; srcRow += 1) {
      ...
      el.className = "loom-selection-row";
