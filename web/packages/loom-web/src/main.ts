// Production entry for the loomtty browser terminal.
//
// Unlike @loom/demo (dev-time, talks to the @loom/bridge on a hardcoded
// localhost port), this build is served by loom-server's own `[web]`
...
import "@loom/app/chrome.css";
import { LoomApp } from "@loom/app";

// Server-side sentinel: "attach to the most-recent / a new session".
// Mirrors `AUTO_SESSION` in crates/loom-server/src/daemon/connection.rs.
...
const TOKEN_KEY = "loom.web.token";

// Same-origin WS endpoint, derived RELATIVE to the directory the page is
// served from (not the origin root). Directly off loom-server the page is
// at `/`, so this is `/ws`; behind a reverse proxy that mounts the bundle
// under a prefix (e.g. `/loom/`, which the relative-asset `base: './'`
// build already supports), the page is at `/loom/` and this becomes
// `/loom/ws` — so the proxy strips the prefix and forwards `/ws` to the
...
function connect(token: string): void {
  ...
  (window as unknown as { __loom?: LoomApp | null }).__loom = app;
}
