#!/usr/bin/env node
// Build the production web bundle (@loom/web) and copy it into the
// directory loom-server serves the SPA from.
//
// loom-server resolves `web.static_dir` (empty → `<server-exe-dir>/web`)
...
// dev server binary (`target/debug/loomtty-server`), i.e. the default
...
console.log(
  `✓ done.\n` +
    `  Start it with:  loomtty web\n` +
    `  (or, pointing the server straight at this dir:\n` +
    `     loomtty-server --headless --web --web-static-dir "${dest}")`,
);
